...
router.post('/signup2', async (req, res) =>{
    const { email, password, tipo, id} = req.body;
    const newLogin = new Login({email, password, tipo, id});
    await newLogin.save();
    
    const token = jwt.sign({_id: newLogin._id}, 'secretkey');
    // console.log(id)
    // console.log(newLogin.id)
    const id1=newLogin._id;
    // res.status(200).json({token, tipo})
    return res.status(200).json({token, tipo, id1, email});
});
...
router.post('/signin2', async (req, res) =>{
    ...
    const id= login.id;
    return res.status(200).json({token, tipo, id, email});
});
...
router.get('/obtenerLogin/:id', loginController.obtenerLoginId)
